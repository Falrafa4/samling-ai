...
import { faArrowLeft, faSearch, faTrashCan, faRecycle, faArrowTrendUp, faScaleBalanced, faUsers, faTemperatureHalf, faWind, faChartLine, faXmark, faTruck, faLeaf } from '@fortawesome/free-solid-svg-icons';
...
export default function Tps() {
  ...
  const filteredTps = tpsList.filter(tps => {
    ...
    const matchesSearch = tps.nama.toLowerCase().includes(searchQuery.toLowerCase());
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col text-gray-900 dark:text-gray-100 pb-24 transition-colors duration-200">
      ...
      <main className="flex-1 px-4 md:px-8 py-6 max-w-5xl mx-auto w-full">
        ...
        <div className="flex flex-col md:flex-row gap-4 mb-6">
          <div className="relative flex-grow">
            <FontAwesomeIcon icon={faSearch} className="absolute left-4 top-1/2 transform -translate-y-1/2 text-gray-400 dark:text-slate-500" />
            <input 
              type="text" 
              value={searchQuery}
          ...
          {filteredTps.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-80 text-center">
              <div className="w-16 h-16 bg-gray-50 dark:bg-slate-750 rounded-full flex items-center justify-center mb-3">
                <FontAwesomeIcon icon={faSearch} className="text-gray-300 dark:text-slate-600 text-xl" />
              </div>
            ...
            <div className="divide-y divide-gray-100 dark:divide-slate-750">
              {filteredTps.map(tps => {
                ...
                if (tps.kapasitas >= 90) {
                  ...
                  statusBgClass = "bg-red-50 dark:bg-red-950/20 border-red-200 dark:border-red-900/30";
                  ...
                  statusBgClass = "bg-yellow-50 dark:bg-yellow-950/20 border-yellow-250 dark:border-yellow-900/30";
                  capacityBarColor = "bg-gradient-to-r from-yellow-400 to-yellow-500";
                } else {
                  statusColorClass = "text-green-650 dark:text-emerald-400";
                  statusBgClass = "bg-green-50 dark:bg-green-950/20 border-green-200 dark:border-green-900/30";
                  capacityBarColor = "bg-gradient-to-r from-green-400 to-green-600";
                }

                return (
                  <div key={tps.id} className="p-6 hover:bg-green-50/20 dark:hover:bg-slate-700/20 transition-colors group">
                    <div className="flex flex-col md:flex-row gap-5">
                      ...
                      <div className="flex-grow w-full">
                        <div className="flex items-center justify-between mb-2">
                          ...
                          <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full border text-xs font-bold ${statusBgClass} ${statusColorClass}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${capacityBarColor.split(' ')[1]}`}></span>
                            {tps.status}
                          </span>
                        </div>

                        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs font-semibold text-gray-500 dark:text-gray-400 mb-4">
                          <span><FontAwesomeIcon icon={faRecycle} className="mr-1.5 text-gray-450" />{tps.jenis}</span>
                          <span><FontAwesomeIcon icon={faArrowTrendUp} className="mr-1.5 text-gray-450" />Jarak: {tps.jarak} m</span>
                          <span>Wilayah: {tps.city}</span>
                        </div>

                        {/* Capacity Progress Bar */}
                        <div className="flex items-center gap-3 mb-4">
                          <div className="flex-grow h-2.5 bg-gray-150 dark:bg-slate-700 rounded-full overflow-hidden">
                            <div className={`h-full rounded-full ${capacityBarColor}`} style={{ width: `${tps.kapasitas}%` }}></div>
                          </div>
      ...
      {showDetailModal && selectedTps && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs">
          <div className="absolute inset-0 bg-transparent" onClick={() => setShowDetailModal(false)} />
            ...
            <div className="px-6 py-6 bg-gray-50 dark:bg-slate-900 overflow-y-auto max-h-[calc(90vh-140px)] space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex flex-col items-center justify-center col-span-1">
                  ...
                  <CircularProgress percent={selectedTps.kapasitas} size={150} />
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col gap-4">
                  ...
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex justify-between items-center transition-colors">
                    <div>
                      ...
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 dark:bg-green-950/30 dark:text-emerald-400 mt-2">
                        <span className="w-2 h-2 bg-green-600 rounded-full mr-1.5 animate-pulse"></span> Bagus
                      </span>
                    </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex-1 flex items-center gap-4 transition-colors">
                    ...
                    <div>
                      <h5 className="font-bold text-gray-800 dark:text-white text-md">{selectedTps.jenis}</h5>
                      <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">Terdeteksi otomatis oleh sensor AI Samling</p>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-white dark:bg-slate-800 flex flex-col gap-2 p-5 shadow-xs border border-gray-100 dark:border-slate-750 rounded-2xl transition-colors">
                  <div className="flex items-center gap-2 text-gray-400 dark:text-slate-500">
                    ...
                    <h4 className="text-xs font-bold uppercase tracking-wider">Persentase Volume</h4>
                  </div>
                ...
                <div className="bg-white dark:bg-slate-800 flex flex-col gap-2 p-5 shadow-xs border border-gray-100 dark:border-slate-750 rounded-2xl transition-colors">
                  <div className="flex items-center gap-2 text-gray-400 dark:text-slate-500">
                    ...
                    <h4 className="text-xs font-bold uppercase tracking-wider">Berat Penampungan</h4>
                  </div>
                  <h1 className="text-2xl font-black text-gray-900 dark:text-white mt-1">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-800 p-4 shadow-xs border border-gray-100 dark:border-slate-750 rounded-2xl text-center transition-colors">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Petugas</p>
                  <p className="text-lg font-black mt-0.5 text-gray-800 dark:text-white">4 Org</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 shadow-xs border border-gray-100 dark:border-slate-750 rounded-2xl text-center transition-colors">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Suhu</p>
                  <p className="text-lg font-black mt-0.5 text-gray-800 dark:text-white">32°C</p>
                </div>
                ...
                <div className="bg-white dark:bg-slate-800 p-4 shadow-xs border border-gray-100 dark:border-slate-750 rounded-2xl text-center transition-colors">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase">Gas Metana</p>
                  <p className="text-lg font-black mt-0.5 text-gray-800 dark:text-white">45%</p>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-755 transition-colors">
                ...
                <div className="flex flex-row justify-around items-center">
                  <div className="flex flex-col items-center">
                    ...
                    <p className="text-xs font-bold text-gray-400 dark:text-slate-500 mt-2">1200 kg</p>
                  </div>
                  <div className="flex flex-col items-center">
                    <CircularProgress percent={78} size={110} label="Non-Organik" />
                    <p className="text-xs font-bold text-gray-400 dark:text-slate-500 mt-2">1640 kg</p>
                  </div>
                </div>
              </div>
