...
import { faTruckFast, faClock, faCheckCircle, faEllipsisVertical, faCheck, faTrashCan, faHistory } from '@fortawesome/free-solid-svg-icons';
...
export default function Aktivitas() {
  ...
  const loadTruckActivities = () => {
    ...
    if (stored) {
      ...
      const initial = [
        { id: 1, status: 'ongoing', truckId: 'T-01', route: 'Perumahan Griya Asri -> TPS Pusat', timestamp: new Date(Date.now() - 15 * 60 * 1000).toISOString() },
        { id: 2, status: 'ready', truckId: 'T-03', route: 'Kawasan Industri Candi -> TPS Barat', timestamp: new Date().toISOString() },
        { id: 4, status: 'completed', truckId: 'T-01', route: 'Area Perkantoran -> TPS Timur', timestamp: new Date(Date.now() - 2 * 3600 * 1050).toISOString() },
        { id: 5, status: 'completed', truckId: 'T-04', route: 'Cluster Melati -> TPS Utara', timestamp: new Date(Date.now() - 5 * 3600 * 1000).toISOString() },
        { id: 6, status: 'completed', truckId: 'T-02', route: 'Desa Suko -> TPS Barat', timestamp: new Date(Date.now() - 8 * 3600 * 1000).toISOString() }
      ];
    ...
    const initialRequests = [
      {
        ...
        wasteType: "organik",
        amount: 320,
        deadline: new Date(Date.now() + 2 * 3600 * 1000).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
      },
      {
        id: `req-2`,
        ...
        amount: 850,
        deadline: new Date(Date.now() + 4 * 3600 * 1000).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
      }
    ];
  ...
  const generateRandomRequest = () => {
    const partners = JSON.parse(localStorage.getItem('samling_pro_sidoarjo_v1')) || defaultPartners;
    ...
    const newRequest = {
      ...
      amount: randomKg,
      deadline: deadline.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })
    };
  ...
  const handleCompleteActivity = (id) => {
    const updated = truckActivities.map(act => {
      if (act.id === id) {
        return { ...act, status: 'completed', timestamp: new Date().toISOString() };
      }
      return act;
    });
  ...
  const handleCompleteRequest = (req) => {
    ...
    const newTx = {
      ...
      sampah: [
        {
          tipe: req.wasteType === 'organik' ? 'Limbah Sayuran & Makanan' : 'Sampah Terpilah Industri',
          berat: req.amount,
    ...
    try {
      const txHistory = JSON.parse(localStorage.getItem('samling_transactions')) || [];
        ...
        {currentTab === 'truck' && (
          <div className="space-y-4 max-w-xl mx-auto">
            {truckActivities.length === 0 ? (
              <p className="text-center text-gray-500 dark:text-gray-400 py-10">Tidak ada aktivitas armada.</p>
            ) : (
              <div className="flex flex-col gap-4 border-l-2 border-gray-200 dark:border-slate-700 pl-4 py-2 ml-4">
                {truckActivities.map(item => {
                  const truck = defaultTrucks.find(t => t.id === item.truckId);
                  ...
                  // Style mappings
                  let statusClass = "text-green-700 bg-green-50 border-green-200 dark:bg-green-950/20 dark:text-emerald-400 dark:border-green-900/30";
                  let statusText = "Selesai";
                  let icon = faCheckCircle;
                  
                  if (isOngoing) {
                    statusClass = "text-blue-700 bg-blue-50 border-blue-200 dark:bg-blue-950/20 dark:text-blue-400 dark:border-blue-900/30";
                    statusText = "Dalam Perjalanan";
                    icon = faTruckFast;
                  } else if (isReady) {
                    statusClass = "text-yellow-750 bg-yellow-50 border-yellow-250 dark:bg-yellow-950/20 dark:text-yellow-400 dark:border-yellow-900/30";
                    statusText = "Siap Berangkat";
                    icon = faClock;
                  }

                  return (
                    <div key={item.id} className="relative bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-150 dark:border-slate-700 flex flex-col transition-colors duration-200">
                      
                      {/* Timeline Dot Indicator */}
                      <span className={`absolute -left-[25px] top-7 block h-4.5 w-4.5 rounded-full border-2 border-white dark:border-slate-900 ${
                        isOngoing ? 'bg-blue-500 animate-pulse' : isReady ? 'bg-yellow-500' : 'bg-green-500'
                      }`}></span>

                      <div className="flex justify-between items-start">
                        <div className="flex items-center gap-3">
                          <FontAwesomeIcon icon={icon} className="text-gray-400 dark:text-slate-500 text-lg" />
                          <h4 className="font-bold text-gray-850 dark:text-white">
                            {truck.name} - <span className="font-mono text-sm">{truck.plate}</span>
                          </h4>
                        </div>
                      ...
                      <div className="flex items-center justify-between mt-4 pl-7">
                        ...
                        <span className={`text-[11px] font-bold border px-2.5 py-1 rounded-full ${statusClass}`}>
                          {statusText}
                        </span>
                      </div>
          ...
          <div className="space-y-4 max-w-xl mx-auto">
            ...
            {partnerRequests.length === 0 ? (
              <div className="text-center bg-white dark:bg-slate-800 rounded-2xl p-10 border border-gray-150 dark:border-slate-700 flex flex-col justify-center items-center">
                <p className="text-gray-500 dark:text-gray-400">Tidak ada permintaan aktif dari mitra.</p>
                <button 
              ...
              <div className="flex flex-col gap-4">
                {partnerRequests.map(req => {
                  ...
                  const themeColor = isOrg ? 'text-green-650 bg-green-50/80 border-green-200 dark:bg-green-950/20 dark:text-emerald-400 dark:border-green-900/30' : 'text-cyan-650 bg-cyan-50/80 border-cyan-200 dark:bg-cyan-950/20 dark:text-cyan-400 dark:border-cyan-900/30';
                  
                  return (
                    <div key={req.id} className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-150 dark:border-slate-700 flex items-start gap-4 transition-colors duration-200">
                      ...
                      <div className="flex-grow min-w-0">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="relative">
                            <button 
                              onClick={() => setActiveRequestMenuId(activeRequestMenuId === req.id ? null : req.id)}
                              className="p-1 text-gray-400 hover:text-gray-700 dark:hover:text-gray-300 w-8 h-8 rounded-full hover:bg-gray-50 dark:hover:bg-slate-750 flex items-center justify-center cursor-pointer focus:outline-none"
                            >
                              <FontAwesomeIcon icon={faEllipsisVertical} />
                            </button>
                            ...
                            {activeRequestMenuId === req.id && (
                              <>
                                <div className="fixed inset-0 z-35" onClick={() => setActiveRequestMenuId(null)} />
                                <div className="absolute right-0 mt-1 w-36 bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 rounded-xl overflow-hidden shadow-xl z-40 animate-slide-in">
                                  <button 
                        ...
                        <p className="text-sm text-gray-700 dark:text-gray-300 mt-2 font-medium">
                          Memerlukan pasokan sampah <span className="font-bold">{req.wasteType}</span> sebanyak <span className="font-bold text-brand dark:text-emerald-400">{req.amount} kg</span>.
                        </p>
                        
                        <div className="flex items-center justify-between mt-4 pt-2 border-t border-gray-50 dark:border-slate-750">
                          <span className="text-xs text-gray-400 dark:text-slate-500">
                            Tenggat Hari Ini: <span className="font-mono font-bold text-gray-700 dark:text-slate-300">{req.deadline}</span>
                          </span>
                          <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded border ${themeColor}`}>
                            {req.wasteType}
                          </span>
                        </div>
                      </div>
