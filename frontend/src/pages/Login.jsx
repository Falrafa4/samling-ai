...
import { faLeaf, faUser, faLock, faEye, faEyeSlash, faTruckLoading, faCheck, faTimes } from '@fortawesome/free-solid-svg-icons';
...
export default function Login() {
  ...
  useEffect(() => {
    ...
    if (timeSinceLastReport < COOLDOWN_MS) {
      const remainingTime = Math.ceil((COOLDOWN_MS - timeSinceLastReport) / 1000);
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      ...
      const userFound = users.find(user => 
        (user.email === trimmedUser || user.username === trimmedUser) && user.password === trimmedPass
      );
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex items-center justify-center p-4 transition-colors duration-200">
      ...
      {showToast && (
        <div className="fixed top-5 right-5 z-50 animate-slide-in">
          <div className="bg-white dark:bg-slate-800 border-l-4 border-green-500 shadow-2xl rounded-lg p-4 flex items-center gap-4 min-w-[300px]">
            <div className="bg-green-100 dark:bg-green-900/30 p-2 rounded-full text-green-600 dark:text-green-400">
              <FontAwesomeIcon icon={faCheck} className="text-lg" />
            </div>
            <div>
              <h4 className="font-bold text-gray-800 dark:text-white">Berhasil Melapor!</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">Petugas sedang menuju lokasi Anda.</p>
            </div>
            <button onClick={() => setShowToast(false)} className="ml-auto text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
      ...
      {/* Main Container */}
      <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl overflow-hidden w-full max-w-6xl flex flex-col-reverse md:flex-row min-h-[600px] transition-colors duration-200">
        ...
        <div className="w-full md:w-1/2 bg-gray-50 dark:bg-slate-900/50 p-4 md:p-6 flex flex-col justify-center relative border-t md:border-t-0 md:border-r border-gray-200 dark:border-slate-700">
          <section className="relative rounded-2xl overflow-hidden shadow-sm bg-white dark:bg-slate-800 w-full h-full min-h-[400px] flex flex-col">
            ...
            <main className="absolute flex flex-col items-start justify-center bg-white/95 dark:bg-slate-850/95 backdrop-blur-sm p-3 rounded-xl shadow-lg border border-gray-100 dark:border-slate-700 z-20 top-20 right-4 w-40">
              <p className="font-semibold text-xs text-gray-800 dark:text-gray-200 mb-2 uppercase tracking-wider">Status Sampah</p>
            ...
            <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 z-30 w-full px-4 flex justify-center">
              ...
              >
                <FontAwesomeIcon icon={faTruckLoading} className={cooldownTime > 0 ? "" : "animate-bounce"} />
        ...
        <div className="w-full md:w-1/2 bg-white dark:bg-slate-800 p-8 md:p-12 flex flex-col justify-center">
          <div className="w-full max-w-md mx-auto">
            ...
            <div className="mb-8">
              ...
              <h3 className="text-xl font-semibold text-gray-800 dark:text-white">Login Petugas/Admin Pusat</h3>
              <p className="text-gray-500 dark:text-gray-400 text-sm">Masuk untuk mengelola laporan dan armada.</p>
            </div>

            {/* Form */}
            <form onSubmit={handleLoginSubmit} className={shake ? 'animate-bounce' : ''}>
              {/* Username Input */}
              <div className="mb-5">
                <label className="block text-gray-750 dark:text-gray-300 text-sm font-medium mb-2 ml-1">Email / Username</label>
              ...
              {/* Error Message */}
              <p className={`text-sm mb-4 h-5 mt-1 font-medium ${errorMessage.includes('berhasil') ? 'text-green-500' : 'text-red-500'}`}>
                {errorMessage}
              </p>
              ...
              <div className="flex justify-between items-center text-sm mb-6">
                <label className="flex items-center gap-2 text-gray-600 dark:text-gray-400 cursor-pointer select-none">
                  <input type="checkbox" className="w-4 h-4 text-brand focus:ring-brand rounded border-gray-300 dark:border-slate-600 dark:bg-slate-700" />
                  Ingat Saya
                </label>
                <a href="#" className="text-brand dark:text-emerald-400 hover:underline font-medium">Lupa Password?</a>
              </div>
