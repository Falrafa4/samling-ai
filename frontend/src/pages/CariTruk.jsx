...
import { faSearch, faTrashCan, faMapPin, faMapMarkedAlt, faTruckMoving, faClock, faRoute, faStar, faPaperPlane, faHandsHelping, faDumpster, faBuilding, faRecycle, faWeightHanging, faLocationDot, faCircleCheck, faArrowRight, faTrashAlt, faCheckCircle, faExclamationCircle } from '@fortawesome/free-solid-svg-icons';
...
import { defaultTps, defaultTpa, defaultTrucks, defaultPartners } from '../utils/mockData';
...
export default function CariTruk() {
  ...
  useEffect(() => {
    if (currentStep === 5) {
      ...
      const busyIds = activities.filter(act => act.status === 'ongoing').map(act => act.truckId);
  ...
  const getIndicatorClass = (stepNum, activeRange) => {
    ...
    const isCompleted = currentStep > stepNum || (stepNum === 2 && currentStep > 2) || (stepNum === 3 && currentStep === 5);
  ...
  const filteredTps = defaultTps.filter(t => {
    ...
    const matchesSearch = t.nama.toLowerCase().includes(tpsSearch.toLowerCase());
  ...
  useEffect(() => {
    ...
    tpsChartInstanceRef.current = new Chart(ctx, {
      type: 'line',
      data: {
        labels: ['6hr lalu', '5hr lalu', '4hr lalu', '3hr lalu', '2hr lalu', '1hr lalu', 'Saat Ini'],
        datasets: [{
          label: 'Kapasitas (%)',
          ...
          tension: 0.4,
          pointBackgroundColor: tpsModalDetail.capacity > 90 ? '#ef4444' : tpsModalDetail.capacity > 70 ? '#facc15' : '#22c55e',
          pointRadius: 5
        }]
      },
      options: {
  ...
  const filteredTpa = defaultTpa.filter(t => {
    ...
    const matchesSearch = t.nama.toLowerCase().includes(tpaSearch.toLowerCase());
    return matchesCity && matchesSearch;
  });
  ...
  const getFilteredMitra = () => {
    let list = partnersList.filter(p => 
  ...
  useEffect(() => {
    ...
    if (partnerChartInstanceRef.current) partnerChartInstanceRef.current.destroy();
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-slate-900 flex flex-col text-gray-900 dark:text-gray-100 pb-16 transition-colors duration-200">
      ...
      <Header 
        ...
        rightContent={
          ...
          <div className="flex items-center gap-1 text-[10px] md:text-xs">
            ...
            <div className="w-3 h-px bg-gray-300"></div>
            <span 
              className={getIndicatorClass(4, [5])}
            >
              4. Armada
            </span>
          </div>
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-150 dark:border-slate-750 overflow-hidden min-h-[400px]">
              {filteredTps.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  <p className="text-gray-500 dark:text-gray-400">Data TPS tidak ditemukan.</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-100 dark:divide-slate-750">
                  {filteredTps.map(t => {
                    const isFull = t.status === "Penuh";
                    const statusColor = isFull ? "text-red-500 bg-red-50 dark:bg-red-950/20" : "text-green-500 bg-green-50 dark:bg-green-950/20";
                      ...
                      >
                        <div className="flex-1 w-full min-w-0">
                          <div className="flex items-center justify-between sm:justify-start gap-3 mb-2">
                            <h3 className="text-base font-bold text-gray-800 dark:text-white truncate">{t.nama}</h3>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="flex-grow h-2 bg-gray-150 dark:bg-slate-700 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${capacityBg}`} style={{ width: `${t.capacity}%` }}></div>
                            </div>
          ...
          <section className="animate-fade-in max-w-xl mx-auto space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-150 dark:border-slate-750 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-bold tracking-wider">Titik Jemput</p>
                  <h2 className="text-md font-bold text-gray-850 dark:text-white">{selectedTps.nama || selectedTps.name}</h2>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-xs flex flex-col gap-4 hover:border-green-500 hover:shadow-md transition-all duration-200">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-extrabold text-lg text-gray-900 dark:text-white">Kirim ke TPA</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Buang sampah langsung ke Tempat Pembuangan Akhir (Landfill).</p>
                  </div>
                </div>
                <button 
              ...
              <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-200 dark:border-slate-700 shadow-xs flex flex-col gap-4 hover:border-green-500 hover:shadow-md transition-all duration-200">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-extrabold text-lg text-gray-900 dark:text-white">Kirim ke Mitra Pengolah</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400">Kirim sampah terpilah ke mitra daur ulang / industri.</p>
                  </div>
                </div>
                <button 
          ...
          <section className="animate-fade-in space-y-4 max-w-4xl mx-auto">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm border border-gray-150 dark:border-slate-750 overflow-hidden min-h-[400px]">
              {filteredTpa.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-64 text-center">
                  <p className="text-gray-500 dark:text-gray-400">Data TPA tidak ditemukan.</p>
                </div>
              ) : (
                <div className="divide-y divide-gray-100 dark:divide-slate-750">
                  {filteredTpa.map(tpa => {
                    ...
                    return (
                      ...
                      >
                        <div className="flex-1 w-full min-w-0">
                          <div className="flex items-center justify-between sm:justify-start gap-3 mb-2">
                            <h3 className="text-base font-bold text-gray-800 dark:text-white truncate">{tpa.nama}</h3>
                            <span className={`px-2.5 py-0.5 rounded-full text-xs font-bold ${statusColor}`}>
                              {tpa.status}
                            </span>
                          </div>
                          ...
                          <div className="flex flex-wrap gap-x-4 text-xs font-semibold text-gray-400 dark:text-slate-500 mb-3">
                            ...
                            <span>Jarak: {(tpa.jarak/1000).toFixed(1)} km</span>
                          </div>

                          <div className="flex items-center gap-3">
                            <div className="flex-grow h-2 bg-gray-150 dark:bg-slate-700 rounded-full overflow-hidden">
                              <div className={`h-full rounded-full ${capacityBg}`} style={{ width: `${tpa.kapasitas}%` }}></div>
                            </div>
                            <span className="text-xs font-extrabold text-gray-700 dark:text-white w-8 text-right">
                              {tpa.kapasitas}%
                            </span>
            ...
            <div className="flex flex-col md:flex-row gap-4 justify-between items-stretch md:items-center">
              ...
              <div className="flex flex-row gap-2 items-center w-full md:w-auto">
                <div className="search-container bg-white dark:bg-slate-800 border border-gray-155 dark:border-slate-700 flex items-center gap-2 px-3 py-2 rounded-xl flex-1 md:w-60 transition-colors">
                  <FontAwesomeIcon icon={faSearch} className="text-gray-450 text-sm" />
                  <input 
                    type="text" 
                    value={mitraSearch}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
              ...
              <div className="lg:col-span-2 space-y-3">
                <div className="flex justify-between items-center mb-1">
                  <h3 className="text-md font-bold text-gray-850 dark:text-white">Pilih Mitra Tujuan</h3>
                  <span id="mitra-result-count" className="text-xs font-semibold text-gray-400 bg-white dark:bg-slate-800 shadow-sm px-2 py-1 rounded-md">
              ...
              <div className="flex flex-col gap-4">
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-150 dark:border-slate-750">
                  <h4 className="text-gray-900 dark:text-white font-bold mb-3">Ringkasan Kategori</h4>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-55 dark:bg-slate-750 p-3 rounded-lg text-center">
                      ...
                      <p className="text-xl font-bold text-gray-900 dark:text-white mt-1" id="mitra-summary-count">
                    ...
                    <div className="bg-gray-55 dark:bg-slate-750 p-3 rounded-lg text-center flex flex-col justify-center">
                      ...
                      <p className={`text-xs font-black uppercase mt-1 ${mitraCategory === 'organik' ? 'text-green-500' : 'text-cyan-500'}`} id="mitra-summary-category">
                        {mitraCategory}
                      </p>
                    </div>
                  </div>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-5 shadow-sm border border-gray-150 dark:border-slate-750">
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">Proporsi daya tampung per mitra</p>
            ...
            <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-150 dark:border-slate-750 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[9px] text-gray-400 dark:text-slate-500 uppercase font-bold tracking-wider">Titik Jemput</p>
                    <h2 className="text-md font-bold text-gray-800 dark:text-white leading-none">{selectedTps.nama || selectedTps.name}</h2>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-[9px] text-gray-400 dark:text-slate-500 uppercase font-bold tracking-wider">Tujuan Akhir</p>
                    <h2 className="text-md font-bold text-gray-855 dark:text-white leading-none">{finalDestination.nama || finalDestination.name}</h2>
                  </div>
                </div>
              </div>
              <div className="text-left sm:text-right shrink-0 border-t sm:border-t-0 pt-2 sm:pt-0 w-full sm:w-auto flex sm:block justify-between">
                <p className="text-xs text-gray-400 uppercase font-bold tracking-wider">Kapasitas TPS</p>
                <p className="text-md font-extrabold text-brand dark:text-emerald-400 mt-1">{selectedTps.kapasitas || selectedTps.capacity}%</p>
              </div>
            </div>
                ...
                return (
                  <div 
                    ...
                    className={`bg-white dark:bg-slate-800 p-5 rounded-2xl border border-gray-150 dark:border-slate-750 shadow-xs flex flex-col gap-4 relative overflow-hidden transition-all ${
                      !isAvailable ? 'opacity-60' : 'hover:border-green-500 hover:shadow-md'
                    }`}
                  >
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-4">
                        <div className={`w-14 h-14 rounded-full ${!isAvailable ? 'bg-gray-100 dark:bg-slate-700 text-gray-400 dark:text-slate-500' : 'bg-green-100 dark:bg-green-950/30 text-green-600 dark:text-emerald-400'} flex items-center justify-center text-xl shrink-0`}>
                          <FontAwesomeIcon icon={faTruckMoving} />
                        </div>
                        <div>
                          <h4 className="font-bold text-lg text-gray-900 dark:text-white">{truck.driver}</h4>
                          <p className="text-sm text-gray-500 dark:text-gray-400 font-mono">{truck.plate}</p>
                        </div>
                      </div>
                      <span className={`text-xs font-bold border px-3 py-1 rounded-full ${
                        !isAvailable ? 'text-yellow-600 bg-yellow-50 border-yellow-150 dark:bg-yellow-950/20 dark:border-yellow-900/30' : 'text-green-600 bg-green-50 border-green-150 dark:bg-green-950/20 dark:border-green-900/30'
                      }`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 py-3 border-t border-b border-gray-50 dark:border-slate-750">
                      <div className="text-center">
                        <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-bold">Estimasi</p>
                      ...
                      <div className="text-center border-l border-r border-gray-100 dark:border-slate-750">
                        <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-bold">Jarak</p>
                        <p className="font-bold text-gray-700 dark:text-slate-350 text-xs mt-1">
                      ...
                      <div className="text-center">
                        <p className="text-[10px] text-gray-400 dark:text-slate-500 uppercase font-bold">Rating</p>
                        <p className="font-bold text-gray-700 dark:text-slate-350 text-xs mt-1">
      ...
      {tpsModalDetail && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs">
          <div className="absolute inset-0 bg-transparent" onClick={() => setTpsModalDetail(null)} />
          <div className="relative bg-white dark:bg-slate-800 w-full sm:max-w-4xl rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] z-10 transition-colors duration-200">
            ...
            <div className="px-6 py-5 border-b border-gray-150 dark:border-slate-750 flex items-center justify-between bg-gray-50 dark:bg-slate-850 shrink-0">
              <div className="flex items-center gap-4">
                <button onClick={() => setTpsModalDetail(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                  <FontAwesomeIcon icon={faArrowLeft} className="text-lg" />
                </button>
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white leading-tight">{tpsModalDetail.nama}</h3>
            ...
            <div className="px-6 py-6 bg-gray-50 dark:bg-slate-900 overflow-y-auto max-h-[calc(90vh-140px)] space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex flex-col items-center justify-center col-span-1">
                  <h4 className="text-xs font-bold text-gray-450 dark:text-slate-500 uppercase tracking-wider mb-2 w-full text-left">Status Saat Ini</h4>
                  <CircularProgress percent={tpsModalDetail.capacity} size={140} />
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col gap-4">
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex justify-between items-center">
                    <div>
                      <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">Kesehatan Sensor</p>
                      <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800 dark:bg-green-950/30 dark:text-emerald-400 mt-2">
                        <span className="w-2 h-2 bg-green-600 rounded-full mr-1.5"></span> Bagus
                      </span>
                    </div>
                    <div className="text-right">
                      <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">Terakhir Diupdate</p>
                      <p className="font-bold text-gray-900 dark:text-white mt-2 text-sm">5 menit lalu</p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white dark:bg-slate-800 p-5 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750 flex-1 flex items-center gap-4">
                    ...
                    <div>
                      <h5 className="font-bold text-gray-800 dark:text-white text-md">{tpsModalDetail.type || tpsModalDetail.jenis}</h5>
                      <p className="text-xs text-gray-400 dark:text-gray-505 mt-0.5">Kategori sampah utama penjemputan</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750">
                <div className="flex justify-between items-center mb-4">
                  <h4 className="text-xs font-bold text-gray-450 dark:text-slate-500 uppercase tracking-wider">Tren Kapasitas (7 Hari Terakhir)</h4>
      ...
      {tpaModalDetail && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-xs">
          <div className="absolute inset-0 bg-transparent" onClick={() => setTpaModalDetail(null)} />
          <div className="relative bg-white dark:bg-slate-800 w-full sm:max-w-3xl rounded-t-3xl sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] z-10 transition-colors duration-200">
            ...
            <div className="px-6 py-5 border-b border-gray-150 dark:border-slate-750 flex items-center justify-between bg-gray-50 dark:bg-slate-850 shrink-0">
              <div className="flex items-center gap-4">
                <button onClick={() => setTpaModalDetail(null)} className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
                  <FontAwesomeIcon icon={faArrowLeft} className="text-lg" />
                </button>
                <div>
                  <h3 className="text-xl font-bold text-gray-900 dark:text-white leading-tight">{tpaModalDetail.nama}</h3>
            ...
            <div className="px-6 py-6 bg-gray-50 dark:bg-slate-900 overflow-y-auto max-h-[calc(90vh-140px)] space-y-6">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-xs border border-gray-100 dark:border-slate-750 text-center">
                  <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">Volume Harian</p>
                  <p className="text-lg font-black text-gray-800 dark:text-white mt-1.5">{tpaModalDetail.dailyVolume.toLocaleString('id-ID')}</p>
                  <p className="text-[10px] text-gray-400 dark:text-slate-550">ton/hari</p>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-xs border border-gray-100 dark:border-slate-750 text-center">
                  <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">Kapasitas</p>
                  <p className={`text-lg font-black mt-1.5 ${tpaModalDetail.kapasitas > 80 ? 'text-red-500' : 'text-green-500'}`}>{tpaModalDetail.kapasitas}%</p>
                  <p className="text-[10px] text-gray-400 dark:text-slate-550">{tpaModalDetail.status}</p>
                </div>
                <div className="bg-white dark:bg-slate-800 p-4 rounded-xl shadow-xs border border-gray-100 dark:border-slate-750 text-center">
                  <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase">Sisa Umur</p>
                  <p className="text-lg font-black text-gray-800 dark:text-white mt-1.5">{tpaModalDetail.lifespan}</p>
                  <p className="text-[10px] text-gray-400 dark:text-slate-550">tahun</p>
                </div>
              </div>

              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-xs border border-gray-100 dark:border-slate-750">
                <h4 className="text-xs font-bold text-gray-450 dark:text-slate-500 uppercase tracking-wider mb-4">Komposisi Sampah Rata-rata</h4>
